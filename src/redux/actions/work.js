...
import { setCompanyBillingWorks, setDownloadProjectWork, setError, setNotAssignedWorks, setUploadProjectWork, setWorksForUpdate, workRequest } from "../reducers/work";
...
export const getNotAssignedWorks = (sourceLanguage,targetLanguage) => async(dispatch) =>{
    try {
        setHeaders();
        dispatch(workRequest());
        ...
        const res = await axios({
            method:"GET",
            url:`${import.meta.env.VITE_API_URL}/api/megdapadmin/work/notAssignedWorks?sourceLanguage=${sourceLanguage}&targetLanguage=${targetLanguage}`
        })
        const works = res.data.works;
        dispatch(setNotAssignedWorks(works));
    } catch (error) {
        const statusCode = error?.response?.status;
        if(statusCode === 401){
            dispatch(logoutUser())
